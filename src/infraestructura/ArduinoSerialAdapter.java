...
public class ArduinoSerialAdapter implements IHardwareComm {
    ...
    @Override
    public boolean setStadiumMode(String mode) {
        if (mode == null) return false;
        String resp = sendCommand("MODE:" + mode.toUpperCase());
        return !resp.startsWith("ERROR");
    }
    ...
    public boolean setZoneColor(String zone, int r, int g, int b) {
        ...
        String resp = sendCommand("ZONE:" + zone.toUpperCase() + ":" + rr + "," + gg + "," + bb);
        return !resp.startsWith("ERROR");
    }

    @Override
    public boolean setZoneOff(String zone) {
        if (zone == null) return false;
        String resp = sendCommand("ZONE:" + zone.toUpperCase() + ":OFF");
        return !resp.startsWith("ERROR");
    }
}
