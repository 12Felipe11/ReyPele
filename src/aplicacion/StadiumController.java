...
public class StadiumController {
    ...
    public String processCommand(String input) {
        ...
        switch (parts[0].toUpperCase()) {
            ...
            case "SCENE":  return handleScene(parts);
    ...
    /** Iluminacion WS2812B: SCENE PARTIDO|ENTRENAMIENTO|EVENTO|EMERGENCIA|MANUAL|OFF */
    private String handleScene(String[] parts) {
        if (parts.length < 2)
            return "  Uso: SCENE PARTIDO|ENTRENAMIENTO|EVENTO|EMERGENCIA|MANUAL|OFF";
        String name = parts[1].toUpperCase();
        switch (name) {
            case "PARTIDO":
            case "ENTRENAMIENTO":
            ...
            case "MANUAL":
            case "OFF":
                boolean ok = facade.setStadiumLightingMode(name);
                return ok
                    ? "  Escena WS2812B: " + name
                    : "  No se pudo aplicar la escena (hardware?).";
            default:
                return "  Escena invalida. Opciones: PARTIDO, ENTRENAMIENTO, EVENTO, EMERGENCIA, MANUAL, OFF";
        }
    }
    ...
    private String getHelp() {
        return "  --- COMANDOS (ReyPele HC-SR04 + WS2812B) ---\n"
             + "  READ                            Leer sensores\n"
             + "  LIGHT <0-100|ON|OFF>            Brillo global de la tira\n"
             + "  ALARM ON|OFF                   Control de alarma\n"
             + "  MODE AUTO|MANUAL|EMERGENCY      Estrategia automatica\n"
             + "  SCENE <P|ENT|EVE|EMER|MAN|OFF>  Escena WS2812B\n"
