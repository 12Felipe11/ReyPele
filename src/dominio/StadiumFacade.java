...
public class StadiumFacade {
    ...
    /** Cambia el modo de iluminacion WS2812B-8 (PARTIDO, ENTRENAMIENTO, EVENTO, EMERGENCIA, MANUAL, OFF). */
    public synchronized boolean setStadiumLightingMode(String mode) {
        return hardware.setStadiumMode(mode);
    }

    /** Asigna color RGB a una zona del estadio (NORTE, ORIENTAL, CANCHA, OCCIDENTAL, SUR). */
    public synchronized boolean setZoneColor(String zone, int r, int g, int b) {
        return hardware.setZoneColor(zone, r, g, b);
    }

    /** Apaga una zona del estadio. */
    public synchronized boolean setZoneOff(String zone) {
        return hardware.setZoneOff(zone);
    }
